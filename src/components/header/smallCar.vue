<template>
    <ul class="nav-aside" v-show="show">
						<!--active-->
						<li class="nav-cart">
								...
								<div class="nav-cart-list">
									...
									<div class="full" v-show="shopCarList.length">
										<div class="nav-cart-items">
											<ul>
												<li class="clear" v-for="item in shopCarList" :key="item.sku_id">
...
export default {
	...
	data(){//先写一个规则
		return{
			noShow:['/car','/chekout'],//两个路径，其中要一个不显示出来
			show:true
		}
	},
	watch:{
	...
	created(){
		...
		let ids = shopCarList.map((item)=>{//如果对象存在从对象里面拿到shopCarList
			return item.sku_id;//找到id
		}).join(',');//转成以逗号隔开的形式
		this.$store.dispatch('getShopCarListInfoAction',{//发请求
			ids
		})
	}
}
